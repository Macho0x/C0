# C0 Language Overview

## What C0 is

C0 is a compiled, statically typed programming language with ML-family syntax that targets Go. It is designed for programmers who want the type safety, pattern matching, and inference of OCaml or F#, but who also want Go's runtime, concurrency model, standard library, and deployment characteristics.
...
2. **Compiles to idiomatic Go.** The output must be readable and maintainable by Go programmers. C0 is not a runtime layer; it is a source-to-source compiler.
3. **Eat our own dog food.** The C0 compiler itself is written in Go, demonstrating that the target ecosystem is suitable for real software.
...
5. **Interop is the point.** C0 code must be able to call Go libraries, and Go code must be able to call the Go emitted by C0.
6. **Incremental adoption.** Teams can introduce C0 file-by-file and feature-by-feature.

## Relationship to other languages

| Language | What C0 borrows | What C0 rejects |
|---|---|---|
...
> **The Go emitted by the C0 compiler must be the kind of Go a senior Go engineer would write by hand if asked to express the same program in idiomatic Go.**
...
- C0 does not ship features that evaporate at the Go boundary (capabilities, effects, ownership checks that don't survive lowering).
